<nav class="main-nav">
	<a href="<?php echo $base_url; ?>" class="logotype" title="Voltar à Home">
		<img src="<?php echo $base_url; ?>skin/img/logotipo-bellaco-h.png" alt="<?php echo $app_name; ?>">
	</a>
	<button class="btn text-black p-3 menu-toggler d-block d-lg-none" onclick="togglemenu()"><i class="fas fa-bars"></i></button>
	<ul class="menu-list list-unstyled mb-0 p-5 p-lg-0" onclick="togglemenu()">
		<li class="nav-item active"><a class="btn mb-3 mb-lg-0 me-0 me-lg-3" href="#">Home</a></li>
		<li class="nav-item"><a class="btn mb-3 mb-lg-0 me-0 me-lg-3" href="#">A Empresa</a></li>
		<li class="nav-item"><a class="btn mb-3 mb-lg-0 me-0 me-lg-3" href="#">Produtos</a></li>
		<li class="nav-item"><a class="btn mb-3 mb-lg-0 me-0 me-lg-1 btn-outline-primary font-weight-bold" href="#">Orçamento</a></li>
		<li class="nav-item"><a class="btn mb-3 mb-lg-0 me-0" href="#"><i class="fab fa-whatsapp"></i></a></li>
	</ul>
</nav>

<div class="menu-backdrop d-lg-none"></div>